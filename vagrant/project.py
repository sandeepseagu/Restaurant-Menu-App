...
from flask import session as login_session # This session is flask pre-defined  session function
...
session = DBSession()# this session like variable
...
def showMenu(restaurant_id):
    restaurant = session.query(Restaurant).filter_by(id = restaurant_id).one_or_none()
    user=session.query(User).filter_by(id = restaurant.user_id).one_or_none()
    creator = getUserInfo(restaurant.user_id)
    items = session.query(MenuItem).filter_by(resturant_id = restaurant_id).all()
...
def newMenuItem(restaurant_id):
  ...
  if request.method == 'POST':
      newItem = MenuItem(name = request.form['name'], description = request.form['description'], price = request.form['price'], course = request.form['course'], resturant_id = restaurant_id, user_id=restaurant.user_id)
      session.add(newItem)
      session.commit()
